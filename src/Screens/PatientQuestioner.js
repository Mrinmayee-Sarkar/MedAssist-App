...
export default function PatientQuestioner() {
  ...
  const handleSubmit = async () => {
    var body = {
      ...
      ansdata: JSON.stringify(answers),
    };
  ...
  const handleCommunicationMethodSelect = (method) => {
      ...
      <div className={classes.container}>
        <div className={classes.header}>
          <div className={classes.text1}>
            Patient Questioner Index {index + 1}/{question?.length}
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div className={classes.text2}>
            Ques{index + 1}:{question[index]?.question?.question}
          </div>
        </div>
        <div className={classes.box}>
          ...
          >
            {" "}
            {question[index]?.subquestiontext}
          </div>
          {question[index]?.subquestion ? (
            <Options
              options={question[index]?.subquestion}
        ...
        >
          ...
          {paymentCompleted && !selectedCommunicationMethod && (
            <div>
             <button onClick={() => handleCommunicationMethodSelect('video')}>Video Call</button>
            <button onClick={() => handleCommunicationMethodSelect('phone')}>Phone Call</button>
            </div>
          )}
          {selectedCommunicationMethod === 'video' && (
            <VideoCall channel={`patient-${patientdata.username}`} />
          )}
        </div>
      </div>
    );
