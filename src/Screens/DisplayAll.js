...
import { getData, imageURL, postData } from '../Services/FetchDjangoServices';
...
export default function DisplayAll() {
  ...
  const fillStates = () => {
    return states.map((item) => {
      return <MenuItem key={item.id} value={item.id}>{item.statename}</MenuItem>;
    });
  };

  const fillCategory = () => {
    return category.map((item) => {
      return <MenuItem key={item.id} value={item.id}>{item.categoryname}</MenuItem>;
    });
  };
  ...
  const handleDelete = (rowData) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        var result1 = await postData('doctordelete', body)
        if (result1.status) {
